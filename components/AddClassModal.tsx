...
export const AddClassModal: React.FC<Props> = ({
  ...
  const onSave = () => {
    ...
    const maxClassId = Math.max(...(classes?.map((x) => Number(x.id)) || [0]));
      ...
      bodyContent={
        <div>
          ...
          <FormElement
            ...
            label="Weekday"
            type="weekday"
            setValue={(val) => setFormData({ ...formData, weekday: val })}
          ...
          <FormElement
            ...
            type="calendar"
            setValue={(val) =>
              setFormData({ ...formData, startDate: new Date(val) })
            }
          ...
          <FormElement
            name="stripeId"
            label="Stripe Id"
            setValue={(val) => setFormData({ ...formData, stripeId: val })}
